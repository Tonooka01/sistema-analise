    ...
    try:
        ...
        rows = conn.execute(f"""
            SELECT
                ...
                SUM(CASE WHEN Status = 'Cancelado' AND CAST(Motivo_cancelamento AS TEXT) != '2' AND DATE(Vencimento) < DATE('now') THEN Valor ELSE 0 END)                             AS canc_vencidos,
                SUM(CASE WHEN Status = 'Cancelado' AND (Motivo_cancelamento IS NULL OR CAST(Motivo_cancelamento AS TEXT) != '2') AND DATE(Vencimento) >= DATE('now') THEN Valor ELSE 0 END) AS cancelados
            FROM Contas_a_Receber
