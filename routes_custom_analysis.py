...
def api_late_interest_analysis():
    ...
    @custom_analysis_bp.route('/custom/churn_prediction', methods=['POST'])
def churn_prediction():
    try:
        filters = request.json.get('filters', {})
        df_vendas, df_financeiro, _, _ = get_data_with_cache()
        
        # Filtra vendas ativas
        df_vendas = apply_date_filter(df_vendas, filters, 'data_venda')
        
        # Lógica simplificada de churn (exemplo)
        # Calcula taxa de cancelamento por mês
        if not df_vendas.empty:
            df_vendas['mes_venda'] = df_vendas['data_venda'].dt.to_period('M')
            churn_rate = df_vendas[df_vendas['status'] == 'Cancelado'].groupby('mes_venda').size()
...
@custom_analysis_bp.route('/custom/real_retention_paid', methods=['POST'])
def real_retention_paid():
    conn = get_db()
    try:
        data = request.get_json() or {}
        filters = data.get('filters', {})
        
        # Filtros de data (opcionais, aplica na data da venda para pegar a safra)
        start_date = filters.get('startDate')
        end_date = filters.get('endDate')
        
        params = []
        where_clauses = ["c.Data_pagamento IS NOT NULL", "c.Vencimento IS NOT NULL"]
        
        if start_date:
            where_clauses.append("v.Data_venda >= ?")
            params.append(start_date)
        if end_date:
            where_clauses.append("v.Data_venda <= ?")
            params.append(end_date)
        ...
        # Query SQL adaptada para SQLite
        # 1. StatsCliente: Calcula a média de dias (Pagamento - Vencimento) por cliente
        #    e conta quantos meses foram pagos.
        # 2. Select final: Agrupa por 'meses_pagos' e tira a média das médias dos clientes.
        
        query = f"""
            WITH StatsCliente AS (
                SELECT 
                    c.id_venda,
                    COUNT(*) as meses_pagos,
                    AVG(JULIANDAY(c.Data_pagamento) - JULIANDAY(c.Vencimento)) as media_atraso_cliente
                FROM Contas_a_Receber c
                JOIN Vendas v ON c.id_venda = v.id_venda
                WHERE {where_sql}
                GROUP BY c.id_venda
            )
            SELECT 
                ...
                AVG(media_atraso_cliente) as media_geral_pagamento
            FROM StatsCliente
            GROUP BY meses_pagos
            ORDER BY meses_pagos
        """
        ...
        result = []
        for row in rows:
            # Tenta acessar como dict (se row_factory estiver ativo) ou tupla
            try:
                meses = row['meses_pagos']
                qtd = row['qtd_clientes']
                media = row['media_geral_pagamento']
            ...
            result.append({
                'meses': meses,
                'count': qtd,
                'avg_payment_delay': media if media is not None else 0.0
            })
